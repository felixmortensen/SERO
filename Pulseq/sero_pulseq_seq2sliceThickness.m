function [slt, W] = sero_pulseq_seq2sliceThickness(seq, dz)
% function [slt, W] = sero_pulseq_seq2sliceThickness(seq, dz)
...
for i = 1:nEvents
    ...
    if cEvent(2) && cEvent(5) && cEvent(7)
        ...
        sha = sero_pulseq_array2sha([abs(rfWF(:)); angle(rfWF(:)); rfT(:); gzA]);

        indLib = find(ismember(Lib(:,1),{sha}));

        if isempty(indLib)
            [M, z, sliceThick] = sero_pulseq_sliceProfile(rfWF, rfT, 90, gzA, [], 1);

            Mc  = M(1,:) + M(2,:)*1i;

            weight = sero_srr_sliceProf2weight(Mc, z, dz, sliceThick, 0);
