function fn_nii = sero_recon_data2nii_adaptiveCombine(fn_data, fn_seq, fn_nii, xps_fn, do_norm)
% function fn_nii = sero_recon_data2nii_adaptiveCombine(fn_data, fn_seq, fn_nii, do_norm)
...
if nargin < 3 || isempty(fn_nii)
    fn_nii = sero_recon_fn_data2nii_aco(fn_data);
end
...
img_mc = sero_recon_recon2DEPI_siemens_per_bval(fn_data, fn_seq, xps_fn, 'auto');
...
for i = 1:numel(unique_b)
    ...
    if numel(im_idx) ~= size(img_mc{i}, 4)
        error('Mismatch: Found %d images with b = %g but reconstructed volume has %d images.', ... %dimensionality check
...
img_co = sero_recon_coilCombineAdaptive_multiShot(img_mc);
