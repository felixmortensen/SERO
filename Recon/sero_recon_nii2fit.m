function fn_par = sero_recon_nii2fit(nii_fn, xps_fn, mask_fn, reg_str, do_t1, mode, new_nii, opt)
% function fn_par = srp_nii2fit(nii_fn, xps_fn, mask_fn, reg_str, do_t1, mode, opt)
...
xps = sero_recon_xps_load(xps_fn);
...
for i = 1:siz(1)
    for j = 1:siz(2)
        ...
        tic
        x0 = sero_recon_setGuess(S, TR, B, W, do_t1);
        x0(:,1) = x0(:,1)/(max(x0(:,1)) + eps); %avoid division with zero
        toc
        
        tic
        T = sero_recon_data2fit_1d_reg_v2(S, TR, B, W, do_t1, reg_str, mode, x0, opt);
        toc
