function [Ez, D] = sero_recon_fit2signalError(m, TR, B, W, S)
% function Ez = sero_recon_fit2signalError(m, TR, B, W, S)
%
...
if nargin < 1 % example
    snr = 100;
    [ts, n, z, a] = sero_recon_default_image_pars();

    % create object
    T = sero_recon_generate_structure(z, 1);

    % Create SERO sampling matrix
    [~, TR,  B,  W ] = sero_recon_sampling_v3(ts, n, a, z);
    TR(isnan(TR)) = 0;

    % Get signal
    S   = sero_recon_fit2data(T, TR,  B,  1, W);

    % Add noise to signal
    S   = sero_recon_add_noise(S,  snr);

    % Estimate parameters
    m  = sero_recon_data2fit_1d (S,  TR,  B,  W,  1, [], [], []);
    ...
    % Run function
    tic
    [Ez, D] = sero_recon_fit2signalError(m, TR, B, W, S);
    toc
    % get position
    p  = sero_recon_tr2position(TR);
...
% Estimated signal
E  = sero_recon_fit2data(m, TR, B, 1, W);
