function sero_recon_simulation_1D_v2(snrRange, nObj, nSims, ar, reg_str, mode, checkpointFileInput)
    % srp_simulation_1D_v2 runs simulations over a range of SNR values, objects, and inner simulations.
    %
    % Inputs:
    ...
    [S, D, T1, V] = sero_recon_generate_structure(numPositions, nObj);
    S = S ./ mean(S, 1);  % Normalize object intensity
    
    %% Create sampling schemes for each method
    [samplingDirect.t, samplingDirect.tr, samplingDirect.b, samplingDirect.w] = sero_recon_sampling_direct(timeStep, numSamples, numPositions);
    [samplingSlice.t, samplingSlice.tr, samplingSlice.b, samplingSlice.w] = sero_recon_sampling_conventional_slice_shift(timeStep, numSamples, ar, numPositions);
    [samplingSero.t, samplingSero.tr, samplingSero.b, samplingSero.w] = sero_recon_sampling_v3(timeStep, numSamples, ar, numPositions);
        ...
        for objIndex = startObject : nObj
            try
                ...
                for simIndex = 1:nSims
                    ...
                    dataDirect = sero_recon_fit2data(trueValues, samplingDirect.tr, samplingDirect.b, 1, samplingDirect.w);
                    dataSlice  = sero_recon_fit2data(trueValues, samplingSlice.tr, samplingSlice.b, 1, samplingSlice.w);
                    dataSero   = sero_recon_fit2data(trueValues, samplingSero.tr, samplingSero.b, 1, samplingSero.w);
                    
                    % Add noise based on the current SNR value.
                    noisyDataDirect = sero_recon_add_noise(double(dataDirect), currentSNR);
                    noisyDataSlice  = sero_recon_add_noise(double(dataSlice), currentSNR);
                    noisyDataSero   = sero_recon_add_noise(double(dataSero), currentSNR);
                    
                    % Perform fitting using the provided regularization strength and regularization mode.
                    tic;
                    fitDirect = sero_recon_data2fit_1d_reg_v2(noisyDataDirect, samplingDirect.tr, samplingDirect.b, samplingDirect.w, 0, reg_str, mode);
                    fitSlice  = sero_recon_data2fit_1d_reg_v2(noisyDataSlice, samplingSlice.tr, samplingSlice.b, samplingSlice.w, 0, reg_str, mode);
                    fitSero   = sero_recon_data2fit_1d_reg_v2(noisyDataSero, samplingSero.tr, samplingSero.b, samplingSero.w, 1, reg_str, mode);
                    toc;
